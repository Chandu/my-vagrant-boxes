...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
	...
	VAGRANT_BASE_BOX_PATH=Pathname(__FILE__).dirname.join('boxes', 'precise64.box').to_s

	# Every Vagrant virtual environment requires a box to build off of.
	config.vm.boot_timeout=1000	
	config.vm.box = "rubuntu"
	config.vm.hostname = "rubuntu.dev"
	config.vm.box_url = "file://" + VAGRANT_BASE_BOX_PATH
	config.vm.network "private_network", ip: "10.0.0.4"
	# The url from where the 'config.vm.box' box will be fetched if it
	# doesn't already exist on the user's system.
	# config.vm.box_url = "file://domain.com/path/to/above.box"
	...
	# the file rubuntu.pp in the manifests_path directory.
	#
	...
	# end
 	...
 	VAGRANT_CHEFFILES_PATH = File.expand_path("..",Dir.pwd)
	config.vm.provision :chef_solo do |chef|
